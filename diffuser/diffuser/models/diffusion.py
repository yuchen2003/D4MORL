...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, cond, t):
        x_recon = self.predict_start_from_noise(
            x, t=t, noise=self.model(x, cond, t))  # Unet dont use 'cond'
    ...
    def loss(self, x, *args):  # x -> traj; args -> cond
        batch_size = len(x)
...
class MOGaussianDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, pref_dim, trans_dim, mod_type='bc', n_timesteps=1000, loss_type='l1', clip_denoised=False, predict_epsilon=True, action_weight=1, loss_discount=1, loss_weights=None):
        super().__init__()
        self.horizon = horizon
        self.observation_dim = observation_dim
        self.action_dim = action_dim
        self.transition_dim = trans_dim
        self.rtg_dim = self.pref_dim = pref_dim
        self.model = model
    ...
    def forward(self, cond, batch_size, *args, **kwargs):  # not used in training
        # args -> horizon, return -> Sample(<denoised traj>, <some? value>, <trajs chain>); kwargs -> verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs(-> sample_fn(**))
        return self.conditional_sample(cond, batch_size, *args, **kwargs)

class MOGaussianInvDynDiffusion(nn.Module):  # for traj without action
    pass
