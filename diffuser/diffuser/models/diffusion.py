...
class MOGaussianDiffusion(nn.Module):
    ...
    def loss(self, x, cond, prefs, returns=None):  # x -> traj; args -> cond
        batch_size = len(x)
...
class MOActionDiffusion(nn.Module):
    def __init__(self, model, horizon, cond_M, observation_dim, action_dim, pref_dim, rtg_dim, trans_dim, hidden_dim=128, mod_type='bc', n_timesteps=1000, loss_type='l1', clip_denoised=False, predict_epsilon=True, action_weight=10, loss_discount=0.99, loss_weights=None, returns_condition=False, condition_guidance_w=0.1, ar_inv=False, train_only_inv=False, hist_len=2):
        super().__init__()
        self.horizon = horizon
        self.cond_M = cond_M
        ...
        self.states_mlp = nn.Sequential(
            nn.Linear(self.observation_dim * hist_len, hidden_dim),
            nn.Mish(),
            nn.Linear(hidden_dim, 4 * hidden_dim),
            nn.Mish(),
            nn.Linear(4 * hidden_dim, hidden_dim),
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, returns, states, pref, verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs):
        device = self.betas.device
    ...
    @torch.no_grad()
    def conditional_sample(self, batch_size, returns, states, pref, horizon=None, **sample_kwargs):
        '''
            conditions : [ (time, state), ... ]
        '''
        horizon = horizon or self.horizon
        ...
        return self.p_sample_loop(shape, returns, states, pref, **sample_kwargs)
    ...
    def forward(self, batch_size, returns, states, pref, *args, **kwargs):  # not used in training
        # args -> horizon, return -> Sample(<denoised traj>, <some? value>, <trajs chain>); kwargs -> verbose=True, return_chain=False, sample_fn=default_sample_fn, **sample_kwargs(-> sample_fn(**))
        returns_emb = self.returns_mlp(returns)
        states_emb = self.states_mlp(states)
        pref_emb = self.pref_mlp(pref)
        return self.conditional_sample(batch_size, returns_emb, states_emb, pref_emb, *args, **kwargs)
